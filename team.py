...
def team(team_name) :
    ...
    st.dataframe(df_games_home.iloc[:,[0,3,4,5,7,8,9]])
    # 차트의 표시
    ...
    if st.checkbox('홈 성적 자세히보기') :
        ...
        home_value2 = df_games_home.iloc[:,7:-1].mean()
    ...
    st.dataframe(df_games_away.iloc[:,[0,3,4,5,7,8,9]])
    if st.checkbox('어웨이 성적 자세히 보기') :
        ...
        away_value = df_games_away.iloc[:,7:-1].mean()
